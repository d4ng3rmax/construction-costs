...
from wtforms import StringField, PasswordField, SubmitField, BooleanField, TextAreaField, SelectField, IntegerField, FieldList, FormField
...
class FormCadastro(FlaskForm):
    email = StringField("E-mail", validators=[DataRequired(message="Campo obrigatório"),
                                                       Email(message="O campo deve ser um email válido")])
    senha = PasswordField("Senha", validators=[DataRequired(message="Campo obrigatório")])
    confirmar_senha = PasswordField("Confirmação da Senha",
...
class FormAlterarSenha(FlaskForm):
    senha = PasswordField("Nova Senha", validators=[DataRequired(message="Campo obrigatório")])
    confirmar_senha = PasswordField("Confirmação da Nova Senha",
                                             validators=[DataRequired(message="Campo obrigatório"),
                                                         EqualTo('senha', message="As senhas devem ser iguais")])
...
class FormNovaObra(FlaskForm):
    ...
    data_inicio = StringField('Data de Início', validators=[DataRequired()], default=datetime.now().strftime('%d/%m/%Y'))
...
class FormLancamentoCusto(FlaskForm):
    data_lancamento = StringField("Data", validators=[DataRequired()], default=datetime.now().strftime('%d/%m/%Y'))
    fornecedor_id = SelectField("Fornecedor", coerce=int, validators=[DataRequired()])
    nota_fiscal = StringField("Nota Fiscal", validators=[DataRequired(), validate_nota_fiscal])
    etapa_id = SelectField("Etapa", coerce=int, validators=[DataRequired(), validar_etapa])
    subetapa_id = SelectField("Subetapa", coerce=int, validators=[DataRequired(), validar_etapa])
    material_id = SelectField("Material", coerce=int, validators=[DataRequired()])
    quantidade = StringField("Quantidade", validators=[DataRequired(), validate_numerical_decimal])
    preco_unitario = StringField("Preço Unitário", validators=[DataRequired(), validate_numerical_decimal])
