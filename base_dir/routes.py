    ...
    if form_cadastro.validate_on_submit():
        ...
        try:
            ...
            if usuario_existente:
                flash('Este e-mail já está cadastrado. Escolha outro e-mail.', 'alert-danger')
...
def recuperarsenha():
    ...
    return render_template('recuperarsenha.html', form_recuperar_senha=form_recuperar_senha)
...
def meuperfil():
    ...
    return render_template('meuperfil.html', usuario=usuario, form_alterar_senha=form_alterar_senha)
...
def dashboard():
    ...
    return render_template('dashboard.html', usuario=usuario, n_obras=n_obras, area_construida=area_construida,
                           n_mat=n_mat, n_ser=n_ser, n_div=n_div, n_lanc=n_lanc)
...
def admin():
    ...
    return render_template('admin.html', usuario=usuario, n_usuarios=n_usuarios, n_obras=n_obras)
...
def obras():
    ...
    if form_nova_obra.validate_on_submit():
        ...
        obra = Obra(nome=obra_nome, endereco=obra_endereco, data_inicio=obra_data_inicio, area=obra_area,
        ...
        if obra_existente:
            ...
            return render_template('obras.html', form_nova_obra=form_nova_obra, usuario=usuario)
    ...
    return render_template('obras.html', form_nova_obra=form_nova_obra, usuario=usuario,
                           obra_selecionada=obra_selecionada, prev_page=prev_page)
...
def obraseditar(obra_id):
    ...
    if form_editar_obra.validate_on_submit():
        ...
        if obra_nome == obra_nome_original:
            ...
            flash('Já existe uma obra com esse nome', 'alert-danger')
            return render_template('obraseditar.html', form_editar_obra=form_editar_obra, usuario=usuario)
...
def etapaseditar(etapa_id):
    ...
    if form_editar_etapas.validate_on_submit():
        ...
        etapa_existente = Etapa.query.filter_by(nome=etapa_nome, usuario=usuario).first()
        if etapa_nome == etapa_nome_original:
            pass
        elif etapa_existente:
            flash('Já existe uma etapa com esse nome', 'alert-danger')
            return render_template('etapaseditar.html', form_editar_etapas=form_editar_etapas, usuario=usuario)
...
def subetapas():
    ...
    form_subetapas.etapa_relacionada.choices = [(etapa.id, etapa.nome.capitalize()) for etapa in sorted_etapas]
    ...
    if form_subetapas.validate_on_submit():
        ...
        if subetapa_existente:
            ...
            return render_template('subetapas.html', form_subetapas=form_subetapas, usuario=usuario)
...
def subetapaseditar(subetapa_id):
    ...
    etapas = Etapa.query.all()
    sorted_etapas = sorted(etapas, key=lambda etapa: etapa.nome)
    form_editar_subetapas.etapa_relacionada.choices = [(etapa.id, etapa.nome.capitalize()) for etapa in sorted_etapas]
    ...
    if form_editar_subetapas.validate_on_submit():
        ...
        if subetapa_nome == subetapa_nome_original:
            ...
            flash('Já existe uma subetapa com esse nome', 'alert-danger')
            return render_template('subetapaseditar.html', form_editar_subetapas=form_editar_subetapas, usuario=usuario)
...
def materiais():
    ...
    if form_materiais.validate_on_submit():
        ...
        if usuario.plano == "Básico" and n_materiais >= 10:
            flash('Limite máximo de materiais cadastradas já foi atingido', 'alert-danger')
        ...
        if material_existente:
            ...
            return render_template('materiais.html', form_materiais=form_materiais, usuario=usuario)
    ...
    materiais_ordenados = sorted(usuario.materiais, key=lambda item: (
...
def materiaiseditar(material_id):
    ...
    if form_editar_materiais.validate_on_submit():
        ...
        if material_nome == material_nome_original:
            ...
            flash('Já existe um material com esse nome', 'alert-danger')
            return render_template('materiaiseditar.html', form_editar_materiais=form_editar_materiais, usuario=usuario)
...
def servicos():
    ...
    if form_servicos.validate_on_submit():
        ...
        if servico_existente:
            ...
            return render_template('servicos.html', form_servicos=form_servicos, usuario=usuario)
...
def servicoseditar(servico_id):
    ...
    if form_editar_servicos.validate_on_submit():
        ...
        if servico_nome == servico_nome_original:
            ...
            return render_template('servicoseditar.html', form_editar_servicos=form_editar_servicos, usuario=usuario)
...
def fornecedores():
    ...
    if form_fornecedores.validate_on_submit():
        ...
        # Verificar duplicidade
        fornecedor_existente = Fornecedores.query.filter_by(nome=fornecedor_nome).first()
        if fornecedor_existente:
            flash('Já existe um fornecedor com esse nome', 'alert-danger')
            return render_template('fornecedores.html', form_fornecedores=form_fornecedores, usuario=usuario)
...
@app.route('/cadastro/fornecedores/editar/ID-<int:fornecedor_id>', methods=['GET', 'POST'])
@login_required
def fornecedoreseditar(fornecedor_id):
    ...
    if form_editar_fornecedores.validate_on_submit():
        ...
        if fornecedor_nome == fornecedor_nome_original:
            ...
            return render_template('fornecedoreseditar.html', form_editar_fornecedores=form_editar_fornecedores,
                                   usuario=usuario)
...
def diversos():
    ...
    if form_diversos.validate_on_submit():
        ...
        if diverso_existente:
            ...
            return render_template('diversos.html', form_diversos=form_diversos, usuario=usuario)
...
def diversoseditar(diverso_id):
    ...
    if form_editar_diversos.validate_on_submit():
        ...
        if diverso_nome == diverso_nome_original:
            ...
            return render_template('diversoseditar.html', form_editar_diversos=form_editar_diversos,
                                   usuario=usuario)
...
def obter_subetapas_por_etapa():
    ...
    subetapas_data = [{'id': subetapa.id, 'nome': subetapa.nome} for subetapa in subetapas]
...
def lancamentomateriais():
    ...
    sorted_fornecedores = sorted(usuario.fornecedores, key=lambda fornecedor: fornecedor.nome)
    form_lancamento_custo.fornecedor_id.choices = [(fornecedor.id, fornecedor.nome.title()) for fornecedor in
                                                   sorted_fornecedores]

    sorted_etapas = sorted(usuario.etapas, key=lambda etapa: etapa.nome)
    form_lancamento_custo.etapa_id.choices = ((99999, "Selecione uma etapa..."),) + tuple(
    ...
    form_lancamento_custo.subetapa_id.choices = ((99999, "Selecione uma subetapa..."),) + tuple(
        (subetapa.id, subetapa.nome.capitalize()) for subetapa in sorted_subetapas)

    sorted_materiais = sorted(usuario.materiais, key=lambda item: (
        re.sub(r'\d+\.?\d*', '', item.nome).strip().lower(),
        extract_numerical_part(item.nome)
    ))
    form_lancamento_custo.material_id.choices = [(material.id, material.nome.capitalize()) for material in
                                                 sorted_materiais]
    ...
    if 'adicionar_item' in request.form:
        if form_lancamento_custo.validate_on_submit() and request.method == 'POST':
            ...
            if usuario.plano == "Básico" and n_lanc >= 10:
                flash('Limite máximo de lançamentos cadastradas já foi atingido', 'alert-danger')
                return redirect(url_for('lancamentomateriais'))

            if usuario.plano == "Avançado" and n_lanc >= 2000:
                flash('Limite máximo de itens lançamentos já foi atingido', 'alert-danger')
                return redirect(url_for('lancamentomateriais'))
    ...
    return render_template('lancamentocusto.html', form_lancamento_custo=form_lancamento_custo, usuario=usuario,
                           obra_selecionada=obra_selecionada)


@app.route('/cadastro/lancamento-materiais/editar/ID-<int:lancamentomateriais_id>', methods=['GET', 'POST'])
@login_required
def lancamentomateriaiseditar(lancamentomateriais_id):
    ...
    # Preencher campos choices do formulario
    sorted_fornecedores = sorted(usuario.fornecedores, key=lambda fornecedor: fornecedor.nome)
    form_editar_lancamento_custo.fornecedor_id.choices = [(fornecedor.id, fornecedor.nome.title()) for fornecedor in
                                                   sorted_fornecedores]

    sorted_etapas = sorted(usuario.etapas, key=lambda etapa: etapa.nome)
    form_editar_lancamento_custo.etapa_id.choices = [(etapa.id, etapa.nome.title()) for etapa in
                                                   sorted_etapas]

    subs = Subetapa.query.filter_by(etapa_id=lancamento.etapa_id)
    sorted_subetapas = sorted(subs, key=lambda subetapa: subetapa.nome)
    form_editar_lancamento_custo.subetapa_id.choices = [(subetapa.id, subetapa.nome.title()) for subetapa in
                                                   sorted_subetapas]
    ...
    form_editar_lancamento_custo.material_id.choices = [(material.id, material.nome.capitalize()) for material in
                                                 sorted_materiais]

    if 'adicionar_item' in request.form:
        if form_editar_lancamento_custo.validate_on_submit() and request.method == 'POST':

            data_lancamento = datetime.strptime(form_editar_lancamento_custo.data_lancamento.data, '%d/%m/%Y')
    ...
    # Preencher o formulário com os dados existentes da obra
    form_editar_lancamento_custo.data_lancamento.data = lancamento.data_lancamento.strftime("%d/%m/%Y")
    ...
    return render_template('lancamentocustoeditar.html', form_editar_lancamento_custo=form_editar_lancamento_custo, usuario=usuario,
                           obra_selecionada=obra_selecionada)
...
def lancamentoservicos():
    ...
    # Preencher campos choices do formulario
    sorted_fornecedores = sorted(usuario.fornecedores, key=lambda fornecedor: fornecedor.nome)
    form_lancamento_servico.fornecedor_id.choices = [(fornecedor.id, fornecedor.nome.title()) for fornecedor in
                                                   sorted_fornecedores]

    sorted_etapas = sorted(usuario.etapas, key=lambda etapa: etapa.nome)
    form_lancamento_servico.etapa_id.choices = ((99999, "Selecione uma etapa..."),) + tuple(
        (etapa.id, etapa.nome.capitalize()) for etapa in sorted_etapas)

    sorted_subetapas = sorted(usuario.subetapas, key=lambda subetapa: subetapa.nome)
    form_lancamento_servico.subetapa_id.choices = ((99999, "Selecione uma subetapa..."),) + tuple(
        (subetapa.id, subetapa.nome.capitalize()) for subetapa in sorted_subetapas)

    sorted_servicos = sorted(usuario.servicos, key=lambda item: (
        re.sub(r'\d+\.?\d*', '', item.nome).strip().lower(),
        extract_numerical_part(item.nome)
    ))
    form_lancamento_servico.servico_id.choices = [(servico.id, servico.nome.capitalize()) for servico in
                                                 sorted_servicos]
    ...
    if 'adicionar_item' in request.form:
        if form_lancamento_servico.validate_on_submit() and request.method == 'POST':
            ...
            lancamento = LancamentoServico(data_lancamento=data_lancamento, nota_fiscal=nota_fiscal_lancamento,
                                         quantidade=quantidade_lancamento, preco_unit=preco_unitario_lancamento,
                                         servico_id=servico_lancamento_id, fornecedor_id=fornecedor_lancamento_id,
                                         etapa_id=etapa_lancamento_id, subetapa_id=subetapa_lancamento_id,
                                         obra_id=obra_selecionada.id)
    ...
    return render_template('lancamentoservico.html', form_lancamento_servico=form_lancamento_servico, usuario=usuario,
                           obra_selecionada=obra_selecionada)


@app.route('/cadastro/lancamento-servicos/editar/ID-<int:lancamentoservicos_id>', methods=['GET', 'POST'])
@login_required
def lancamentoservicoseditar(lancamentoservicos_id):
    ...
    # Preencher campos choices do formulario
    sorted_fornecedores = sorted(usuario.fornecedores, key=lambda fornecedor: fornecedor.nome)
    form_editar_lancamento_servico.fornecedor_id.choices = [(fornecedor.id, fornecedor.nome.title()) for fornecedor in
                                                   sorted_fornecedores]

    sorted_etapas = sorted(usuario.etapas, key=lambda etapa: etapa.nome)
    form_editar_lancamento_servico.etapa_id.choices = [(etapa.id, etapa.nome.title()) for etapa in
                                                   sorted_etapas]

    subs = Subetapa.query.filter_by(etapa_id=lancamento.etapa_id)
    sorted_subetapas = sorted(subs, key=lambda subetapa: subetapa.nome)
    form_editar_lancamento_servico.subetapa_id.choices = [(subetapa.id, subetapa.nome.title()) for subetapa in
                                                   sorted_subetapas]
    ...
    form_editar_lancamento_servico.servico_id.choices = [(servico.id, servico.nome.capitalize()) for servico in
                                                 sorted_servicos]

    if 'adicionar_item' in request.form:
        if form_editar_lancamento_servico.validate_on_submit() and request.method == 'POST':

            data_lancamento = datetime.strptime(form_editar_lancamento_servico.data_lancamento.data, '%d/%m/%Y')
            lancamento.data_lancamento = data_lancamento
    ...
    # Preencher o formulário com os dados existentes da obra
    form_editar_lancamento_servico.data_lancamento.data = lancamento.data_lancamento.strftime("%d/%m/%Y")
    ...
    return render_template('lancamentoservicoeditar.html', form_editar_lancamento_servico=form_editar_lancamento_servico, usuario=usuario,
                           obra_selecionada=obra_selecionada)
...
def lancamentodiversos():
    ...
    # Preencher campos choices do formulario
    sorted_diversos = sorted(usuario.diversos, key=lambda item: (
        re.sub(r'\d+\.?\d*', '', item.nome).strip().lower(),
        extract_numerical_part(item.nome)
    ))
    form_lancamento_diverso.diverso_id.choices = [(diverso.id, diverso.nome.capitalize()) for diverso in
                                                 sorted_diversos]
    ...
    if 'adicionar_item' in request.form:
        if form_lancamento_diverso.validate_on_submit() and request.method == 'POST':
            ...
            lancamento = LancamentoDiverso(data_lancamento=data_lancamento,
                                         quantidade=quantidade_lancamento, preco_unit=preco_unitario_lancamento,
                                         diverso_id=diverso_lancamento_id, obra_id=obra_selecionada.id)
    ...
    return render_template('lancamentodiverso.html', form_lancamento_diverso=form_lancamento_diverso, usuario=usuario,
                           obra_selecionada=obra_selecionada)


@app.route('/cadastro/lancamento-diversos/editar/ID-<int:lancamentodiversos_id>', methods=['GET', 'POST'])
@login_required
def lancamentodiversoseditar(lancamentodiversos_id):
    ...
    form_editar_lancamento_diverso.diverso_id.choices = [(diverso.id, diverso.nome.capitalize()) for diverso in
                                                 sorted_diversos]

    if 'adicionar_item' in request.form:
        if form_editar_lancamento_diverso.validate_on_submit() and request.method == 'POST':

            data_lancamento = datetime.strptime(form_editar_lancamento_diverso.data_lancamento.data, '%d/%m/%Y')
            lancamento.data_lancamento = data_lancamento
    ...
    # Preencher o formulário com os dados existentes da obra
    form_editar_lancamento_diverso.data_lancamento.data = lancamento.data_lancamento.strftime("%d/%m/%Y")
    ...
    return render_template('lancamentodiversoeditar.html', form_editar_lancamento_diverso=form_editar_lancamento_diverso, usuario=usuario,
                           obra_selecionada=obra_selecionada)
...
def custoporetapa():
    ...
    for etapa in etapas:
        ...
        lancamento_materiais = LancamentoCusto.query.filter_by(etapa_id=etapa.id, obra_id=obra_selecionada.id).all()
        ...
        lancamento_servicos = LancamentoServico.query.filter_by(etapa_id=etapa.id, obra_id=obra_selecionada.id).all()
        for lancamento in lancamento_servicos:
            lancamento_custo = lancamento.quantidade * lancamento.preco_unit
            custo_etapa[etapa.nome] += lancamento_custo
            custo_total += lancamento_custo

    custo_etapa_ordenado = dict(sorted(custo_etapa.items(), key=lambda item: item[1], reverse=True))

    if custo_total == 0:
        custo_total = 1

    return render_template('custoporetapa.html', custo_etapa=custo_etapa_ordenado, custo_total=custo_total)
...
def custoporsubetapa():
    ...
    for subetapa in subetapas:
        ...
        lancamento_materiais = LancamentoCusto.query.filter_by(subetapa_id=subetapa.id, obra_id=obra_selecionada.id).all()
        for lancamento in lancamento_materiais:
            lancamento_custo = lancamento.quantidade * lancamento.preco_unit
            custo_subetapa[subetapa.nome] += lancamento_custo
            custo_total += lancamento_custo

        lancamento_servicos = LancamentoServico.query.filter_by(subetapa_id=subetapa.id, obra_id=obra_selecionada.id).all()
        for lancamento in lancamento_servicos:
            lancamento_custo = lancamento.quantidade * lancamento.preco_unit
            custo_subetapa[subetapa.nome] += lancamento_custo
            custo_total += lancamento_custo

    custo_subetapa_ordenado = dict(sorted(custo_subetapa.items(), key=lambda item: item[1], reverse=True))

    if custo_total == 0:
        custo_total = 1

    return render_template('custoporsubetapa.html', custo_subetapa=custo_subetapa_ordenado, custo_total=custo_total)
...
def quantitativo():
    ...
    for material in materiais:
        ...
        lancamento_materiais = LancamentoCusto.query.filter_by(material_id=material.id, obra_id=obra_selecionada.id).all()
        for lancamento in lancamento_materiais:
            qtd_material[material.nome] += lancamento.quantidade

    qtd_material_ordenado = dict(sorted(qtd_material.items(), key=lambda item: item[1], reverse=True))

    return render_template('quantitativo.html', qtd_material=qtd_material_ordenado, usuario=usuario)
...
def fluxocaixamensal():
    ...
    if obra_selecionada:
        lancamento_materiais = LancamentoCusto.query.filter_by(obra_id=obra_selecionada.id).all()
        lancamento_servicos = LancamentoServico.query.filter_by(obra_id=obra_selecionada.id).all()
        lancamento_diversos = LancamentoDiverso.query.filter_by(obra_id=obra_selecionada.id).all()
        ...
        meses_ordenados = sorted(custos_mensais.keys(), key=lambda x: datetime.strptime(x, '%m-%Y'))
    ...
    return render_template('fluxocaixamensal.html', custos_mensais=custos_ordenados, usuario=usuario)
...
def resumodecustos():
    ...
    return render_template('resumodecustos.html', custo_materiais=custo_materiais, custo_servicos=custo_servicos,
                           custo_diversos=custo_diversos, custo_total=custo_total, custo_metro=custo_metro)